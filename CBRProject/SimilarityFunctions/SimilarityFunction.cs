...
namespace CBRProject.SimilarityFunctions
{
    public class SimilarityFunction
    {
        ...
        public List<Tuple<double, Order>> GetSimilarity(List<Order> orders,Order newOrder)
        {
            List<Tuple<double, Order>> tuples = new List<Tuple<double, Order>>();
            foreach (var order in orders)
            {
                ...
                sum += 1.0 * GetLinearOrderCount(order.OrderCount, newOrder.OrderCount);
                sum += 5.0 * GetLinearOrderTime(order.OrderTime, newOrder.OrderTime);
                sum += 4.0 * GetLegthOfWay(order.LengthOfWay, newOrder.LengthOfWay);

                tuples.Add(new Tuple<double, Order>(sum, order));
            }
